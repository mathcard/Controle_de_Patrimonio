...
if (isset($_GET['selsala'])) {
    setcookie('auxp',$_GET['selsala'], time() + 60);
 }else{
 $ppredio="";
 $predio="";
 $pred="";
 ...
 if (isset($_COOKIE['auxp'])){
     if (empty($_COOKIE['auxp'])){
         ...
         $ppredio="&selsala=" . $_COOKIE['auxp'];
         $predio=" and sala.numero=" . $_COOKIE['auxp'];
         $pred=$_COOKIE['auxp'];
...
?>
    <div style="margin-left:33%;padding:70px 0">
        ...
        <div class="login-form-1">
            <form id="login-form" class="text-left" action="T11s.php" method="get">
    ...
    <div class='table-responsive col-md-12'>
        <table class='table table-striped'>
            <thead>
                <tr>
                <?php
                    echo "<th><a href='T11s.php?ordem=numero{$pnome}{$dataX}{$ppredio}'>Código</a></th>";
                    echo "<th><a href='T11s.php?ordem=descricao{$pnome}{$dataX}{$ppredio}'>Descrição</a></th>";
                    echo "<th><a href='T11s.php?ordem=datacompra{$pnome}{$dataX}{$ppredio}'>Data da Compra</a></th>";
                    echo "<th><a href='T11s.php?ordem=prazogarantia{$pnome}{$dataX}{$ppredio}'>Garantia</a></th>";
                    echo "<th><a href='T11s.php?ordem=nrnotafiscal{$pnome}{$dataX}{$ppredio}'>Nota</a></th>";
                    echo "<th><a href='T11s.php?ordem=fornecedor{$pnome}{$dataX}{$ppredio}'>Fornecedor</a></th>";
                    echo "<th><a href='T11s.php?ordem=valor{$pnome}{$dataX}{$ppredio}'>Valor</a></th>";
                    echo "<th><a href='T11s.php?ordem=situacao{$pnome}{$dataX}{$ppredio}'>Situação</a></th>";
                    echo "<th><a href='T11s.php?ordem=codcategoria{$pnome}{$dataX}{$ppredio}'>Categoria</a></th>";
                    echo "<th><a href='T11s.php?ordem=numsala{$pnome}{$dataX}{$ppredio}'>Sala</a></th>";
...
if(!empty($_GET['nome'])){
   ...
   (select ba.numero from baixabempatrimonial ba where ba.data {$mm} '{$data10}')) and upper(bp.descricao) like upper('{$nome}')". $predio . $ordem;
    ...
    <div class="etc-login-form">
        ...
        <a href="T11s.php" onClick="SetCookies('aux','','-1'); SetCookies('auxp','','-1');">Listar novamente   </a>
